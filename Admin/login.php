...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Query untuk memeriksa admin dari tabel admin
    ...
    if ($result && mysqli_num_rows($result) === 1) {
        ...
        if ($password === $user['password']) { // Ubah sesuai metode penyimpanan password
            // Set session dengan namespace 'admin'
            $_SESSION['admin'] = [
                'id' => $user['id_admin'],
            ...
            $update_login = "UPDATE admin SET updated_at = NOW() WHERE id_admin = ?";
            $stmt_update = mysqli_prepare($conn, $update_login);
            mysqli_stmt_bind_param($stmt_update, "i", $user['id_admin']);
  ...
  <div class="login-container">
    ...
    <div class="login-card">
      <div class="login-header">
        <h4><i class="fas fa-user-lock me-2"></i>Login Admin BMS</h4>
      </div>
