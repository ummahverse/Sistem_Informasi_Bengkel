...
// Redirect if already logged in as karyawan
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($result && mysqli_num_rows($result) === 1) {
        ...
        // Verifikasi password (sesuaikan dengan sistem penyimpanan password Anda)
        if ($password === $user['password']) {
            // Set session dengan namespace 'karyawan'
            $_SESSION['karyawan'] = [
                'id' => $user['id_karyawan'],
            ...
            $update_login = "UPDATE karyawan SET updated_at = NOW() WHERE id_karyawan = ?";
            $stmt_update = mysqli_prepare($conn, $update_login);
            mysqli_stmt_bind_param($stmt_update, "i", $user['id_karyawan']);
...
<head>
  ...
  <style>
    ...
    body {
      ...
      background-image: url('../images/samping.jpg'); /* Fixed path to parent directory */
  ...
  <div class="login-container">
    ...
    <div class="login-card">
      <div class="login-header">
        <h4><i class="fas fa-user-lock me-2"></i>Login Karyawan BMS</h4>
      </div>
